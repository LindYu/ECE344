...
thread_destroy(struct thread *thread)
{
	...
	if (thread->t_pid) {
		// giveup the current pid
		pid_giveup(thread->t_pid);
	}
...
thread_bootstrap(void)
{
	...
	/* Initialize the first thread's pid */
	me->t_pid = pid_bootstrap();
	if (me->t_pid == NULL) panic("thread_bootstrap: Cannot acquire pid");
...
//		lock_acquire(write_lock);
	...
	/* Acquire new process id */
	newguy->t_pid = pid_create("new process");
	if (newguy->t_pid == NULL) {
		//kfree(newguy->t_name);
	...
	errno  = pid_acquire(newguy->t_pid);
	
	newguy->t_pid->ppid = curthread->t_pid;
	...
	if (curthread->t_vmspace != NULL) {
		errno = as_copy(curthread->t_vmspace, &(newguy->t_vmspace));
...
//	lock_release(write_lock);
 ...
 fail:
	...
	pid_giveup(newguy->t_pid);
	pid_destroy(newguy->t_pid);
	kfree(newguy);
