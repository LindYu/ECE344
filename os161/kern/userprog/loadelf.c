...
load_segment(struct vnode *v, off_t offset, vaddr_t vaddr, 
	     ...
	     int is_executable)
{
//	kprintf("memsize: %d filesize: %d offset %d vaddr 0x%x\n ", memsize, filesize,offset,vaddr);
	struct uio u;
	int result;
	size_t fillamt;
	...
	result = VOP_READ(v, &u);
	if (result) {
	//	kprintf("here, mem is %d,filesize is %d, offset is %d\n",memsize,filesize,offset);
		return result;
	}
...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	for (i=0; i<eh.e_phnum; i++) {
	    ...
	    assert(strlen(curthread->t_vmspace->prog_name) !=0);
		if (i==1) {
			curthread->t_vmspace->offset_1 = ph.p_offset;
			curthread->t_vmspace->vaddr_1 = ph.p_vaddr;
			curthread->t_vmspace->filesize_1 = ph.p_filesz;
			curthread->t_vmspace->is_executable_1 = ph.p_flags & PF_X;
		} else {
			curthread->t_vmspace->offset_2 = ph.p_offset;
			curthread->t_vmspace->vaddr_2 = ph.p_vaddr;
			curthread->t_vmspace->filesize_2 = ph.p_filesz;
			curthread->t_vmspace->is_executable_2 = ph.p_flags & PF_X;
		}
