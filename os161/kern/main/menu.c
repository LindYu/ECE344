...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	/* Modify so that supports passing arguments from menu */
...
*/
	...
	int status;
	int err = sys_waitpid(ret->t_pid->pid, &status, 0, 0);
	if (err) {
...
cmd_df(int nargs, char **args)
{
	...
	if (strcmp(args[2], "on")==0) {
		...
		// set fnth flag to 0
		dbflags &= (~(1<<(fn-1)));
	} else { kprintf("Usage: df nr on/off\n"); return 1;}
